...
let _modelChart = null;   // holds the active Chart.js instance — destroyed before re-render
...
function renderResults(results) {
  ...
  // Chart — always driven from live results
  renderModelChart(results.all_results || []);
}

// ─── Model comparison chart ───────────────────────────────────────────────────
// Metric priority: picks the most meaningful key available for the current task
const CHART_METRIC_PRIORITY = [
  'silhouette_score',   // clustering
  'val_r2',             // regression (validation split)
  'r2',                 // regression (no split)
  'val_accuracy',       // classification (validation split)
  'accuracy',           // classification (no split)
  'f1',                 // classification fallback
];
...
function renderModelChart(models) {
  ...
  // ① Destroy previous instance — prevents stale regression chart bleeding through
  if (_modelChart) {
    _modelChart.destroy();
    _modelChart = null;
  }

  // ② Labels always come from the live results array
  ...
  // ③ Pick primary metric from the actual available keys
  const availableKeys = Object.keys(models[0].metrics);
  const primaryKey =
    CHART_METRIC_PRIORITY.find(k => availableKeys.includes(k)) ||
    availableKeys.find(k => k !== 'fit_time') ||
    availableKeys[0];

  const vals = models.map(m => {
    const v = m.metrics[primaryKey];
    return (v !== null && v !== undefined) ? v : 0;
  });

  const ctx = canvas.getContext('2d');
  _modelChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels,
      datasets: [{
        label: primaryKey.replace(/_/g, ' '),
        data: vals,
        backgroundColor: vals.map((_, i) =>
          i === 0 ? 'rgba(47,255,180,0.8)' : 'rgba(99,102,241,0.5)'
        ),
        borderColor: vals.map((_, i) =>
          i === 0 ? '#2fffb4' : '#6366f1'
        ),
        borderWidth: 1,
        borderRadius: 4,
