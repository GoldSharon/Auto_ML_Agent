...
class DataPrepAgent:
    ...
    def transform(self, new_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for key in ['onehot_encoder', 'frequency_encoder', 'fallback_onehot_encoder']:
            if key in self.transformers:
                try:
                    df = self.transformers[key].transform(df)
                except Exception as e:
    ...
    def _step_3_fit_transform_outliers(self, config: Dict):
        ...
        def _is_winsorizable(col: str) -> bool:
            ...
            return not (series.std() == 0 or (n_unique <= 10 and unique_ratio < 0.05))
    ...
    def _step_4_fit_transform_categorical(self, config: Dict):
        ...
        already_handled = set(onehot_cols + freq_cols)
        leftover_cat = [
            c for c in self.feature_df.select_dtypes(include=['object', 'category']).columns
            if c not in already_handled
        ]
        if leftover_cat:
            if self.verbose:
                print(f"  • Fallback one-hot encoding {len(leftover_cat)} unhandled column(s): {leftover_cat}")
            for col in leftover_cat:
                self.feature_df[col] = self.feature_df[col].astype(str).replace('nan', 'missing')
            drop_last        = self.learning_type == LearningType.CLUSTERING
            fallback_encoder = OneHotEncoder(variables=leftover_cat, drop_last=drop_last)
            self.feature_df  = fallback_encoder.fit_transform(self.feature_df)
            self.transformers['fallback_onehot_encoder'] = fallback_encoder
