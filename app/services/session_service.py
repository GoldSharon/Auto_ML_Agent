...
class SessionService:
    """Single-session state manager with in-memory cache and thread safety."""

    def __init__(self):
        self._base  = settings.session_data_path
        self._lock  = threading.Lock()
        self._cache: Optional[Dict[str, Any]] = None  # in-memory session cache
    ...
    def _load(self) -> Dict[str, Any]:
        """Return cached session dict, loading from disk only on first call."""
    ...
    def _save(self, data: Dict[str, Any]):
        """Write to in-memory cache and flush to disk under lock."""
        with self._lock:
    ...
    def _invalidate_cache(self):
        """Force a fresh disk read on next access (used after clear())."""
        self._cache = None
    ...
    def get_stage(self) -> str:
        return self._load().get("stage", "empty")
    ...
    def init_session(self, project_name: str, filename: str,
                     rows: int, columns: int, column_names: list):
        self._save({
            "stage":        "uploaded",
    ...
    def features_path(self)      -> Path: return self._base / _FEATURES_FILE
    def target_path(self)        -> Path: return self._base / _TARGET_FILE
    def test_features_path(self) -> Path: return self._base / _TEST_FEATURES_FILE
    def test_target_path(self)   -> Path: return self._base / _TEST_TARGET_FILE
    def model_path(self)         -> Path: return self._base / _MODEL_FILE
    def metrics_path(self)       -> Path: return self._base / _METRICS_FILE
    def report_path(self)        -> Path: return self._base / _REPORT_FILE
    def predictions_path(self)   -> Path: return self._base / _PREDICTIONS_FILE
    ...
    def clear(self):
        """Wipe all session data and uploaded files, then reset cache."""
    ...
    def available_downloads(self) -> List[Dict[str, str]]:
        ...
        return [f for f in candidates if Path(f["path"]).exists()]
