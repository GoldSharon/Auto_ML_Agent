...
from app.util.file_handler import open_file
...
def _to_ml_type(val: str) -> MLType:
    return {"Supervised": MLType.SUPERVISED, "Unsupervised": MLType.UNSUPERVISED}.get(val, MLType.SUPERVISED)
...
def _to_learning_type(val: str) -> LearningType:
    ...
    }.get(val, LearningType.CLASSIFICATION)
...
def _sanitize_for_json(obj):
    ...
    if isinstance(obj, pd.Series):          return _sanitize_for_json(obj.tolist())
    return obj
...
class PipelineService:
    ...
    def load_dataset(self, filepath: Path) -> pd.DataFrame:
        """
        Load and canonicalize columns via open_file().
        Results are cached in session so run_analysis() and run_training()
        share the same parsed DataFrame without re-reading disk.
        """
        cache_key = session_service.get("_df_cache_path")
        if cache_key == str(filepath):
            import pickle, base64
            raw = session_service.get("_df_cache_data")
            if raw:
                return pickle.loads(base64.b64decode(raw))

        df, col_mapping = open_file(str(filepath))
        if df is None:
            raise ValueError(
                f"Could not load dataset from {filepath}. "
                "Check that the file format and encoding are supported."
            )

        session_service.set("col_mapping", col_mapping)

        # Cache the parsed DataFrame in session (invalidated on clear/new upload)
        import pickle, base64
        session_service.set("_df_cache_path", str(filepath))
        session_service.set("_df_cache_data", base64.b64encode(pickle.dumps(df)).decode())
    ...
    def run_training(self) -> Dict[str, Any]:
        ...
        df = self.load_dataset(filepath)  # served from cache if already loaded
