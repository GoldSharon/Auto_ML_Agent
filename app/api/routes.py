...
async def run_analysis():
    ...
    try:
        report, prep_config = await run_in_threadpool(pipeline_service.run_analysis)
...
async def run_training():
    ...
    try:
        results = await run_in_threadpool(pipeline_service.run_training)
...
async def predict(body: PredictionRequest):
    ...
    try:
        result  = await run_in_threadpool(pipeline_service.predict, body.features)
...
async def get_feature_names():
    ...
    import pandas as pd
    df = pd.read_csv(feat_path, nrows=0)
